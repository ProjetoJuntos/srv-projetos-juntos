const { beneficiariosCollection } = require('../../repository');

const add = async (request, reply) => {
  try {
    const result = await beneficiariosCollection.insertOne(request.body);
...
const listAll = async (request, reply) => {
  try {
    const result = await beneficiariosCollection.findAll();
    reply.header('Access-Control-Allow-Origin', '*');
    reply.code(200).send(result);
  } catch (error) {
    reply.code(error.statusCode || 500).send(error.message || error);
